...
class TemplateButtonActivate(MDFillRoundFlatButton):
    ...
    def template_activate(self, *args):
        logging.info(f"setting active templ {self.parent.template.name}, layout_id- {self.parent.id}")
        asyncio.create_task(self.parent.BLEClient.set_active_template(self.parent.template))
        

#     def on_button_touch(self, layout_id):
#         if self.id == layout_id:
#             self.template_delete()
# #       this indicates that the event is completed
#         return True
